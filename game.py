...
prompts = {
    ...
    "cases": (
        ...
        "Your task is to compile a list of 250 different life situations that can occur on the street, at school, at home, on the playground, in the store, in class, and so on. "
        "Do not repeat situations and do not indicate any possible solutions in them. Format the answer as a Python list. Here is an example answer with different scenarios: "
        "[\"You are walking down the street with your friend and see someone throwing trash on the sidewalk. What will you do?\", "
...
def extract_list(response: str) -> list:
    if response.startswith("situations = [") and response.endswith("]"):
        return response[len("situations = "):].strip()
    return response
